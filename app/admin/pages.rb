ActiveAdmin.register Manual do
  ...
  after_save do |manual|
    event = params[:manual][:active_admin_requested_event]
    # release publishing
    unless event.blank?
      # whitelist to ensure we don't run an arbitrary method
      safe_event = (manual.aasm_events_for_current_state & [event.to_sym]).first
      raise "Forbidden event #{event} requested on instance #{manual.id}" unless safe_event
      # launch the event with bang
      manual.send("#{safe_event}!")
    end
  end

  menu :label => "Manuals", :parent => "Content", :priority => 4
  ...
  action_item :only => [:show, :edit] do
    link_to "View this page", manual.permalink
  end

  index :title => "Guide" do
    @manuals = Manual.unscoped.all
    render "index"
  end
  ...
  form do |f|
    ...
    f.inputs "Page", :id => "resource_data", :'data-class' => "Page", :'data-id' => manual.id do
      f.input :title

      f.input :parent_id,
              :as => :select,
              :collection => options_for_select([["Root", ""]].concat(Manual.all.map { |p| ["-" * p.depth + " " + p.title, p.id]}), manual.parent_id.nil? ? params[:parent_id] : manual.parent_id ),
