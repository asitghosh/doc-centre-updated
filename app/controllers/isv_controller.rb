class IsvController < ApplicationController
  ...
  def index
    redirect_to Isv.published.roots.order("sortable_order ASC").first.permalink
  end
  ...
  def print_all
    @pages = Isv.printable.find_all{ |p| p.is_guide? }
  end
  ...
  def get_pages(prefix = nil)
    ...
    requested_page = Page.where("type = ? OR type IS NULL", "Isv").find_by_permalink!("/isv-info/#{prefix}#{params[:path]}")
  ...
  def set_sidenav_root(root_page = "manuals")
    @navigation = Page.where("type = ?", "Isv").arrange(:order => :sortable_order)
  end
